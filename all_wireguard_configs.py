...
class Warp():
    ...
    def download_wgcf(self):
        ...
        if not os.path.isfile(self.wgcf_path):
            ...
            # wgcf_cmds = {
            #     "arm64": f'curl -o {wgcf_path} -L "https://github.com/ViRb3/wgcf/releases/download/v2.2.22/wgcf_2.2.22_linux_arm64"',
            #     "amd64": f'curl -o {wgcf_path} -L "https://github.com/ViRb3/wgcf/releases/download/v2.2.22/wgcf_2.2.22_linux_amd64"',
            #     "arm": f'curl -o {wgcf_path} -L "https://github.com/ViRb3/wgcf/releases/download/v2.2.22/wgcf_2.2.22_linux_armv7"',
            #     "386": f'curl -o {wgcf_path} -L "https://github.com/ViRb3/wgcf/releases/download/v2.2.22/wgcf_2.2.22_linux_386"',
            # }
    ...
    def download_warpendpoint(self):
        ...
        if not os.path.isfile(self.warpendpoint_path):
            ...
            warpendpoint_url = f"https://raw.githubusercontent.com/Ptechgithub/warp/main/endip/{self.cpu}"
    ...
    def generate_keys_offline(self):
        def create_wgcf_account():
            print("Register a free account in cloudflare ...")
            max_retry = 5
            while not os.path.isfile("wgcf-account.toml") and max_retry > 0:
                start_time = time.time()
                self.run_command(f"{self.wgcf_path} register --accept-tos")
        ...
        def create_wgcf_profile():
            print("Create a profile for this account ...")
            max_retry = 5
            while not os.path.isfile("wgcf-profile.conf") and max_retry > 0:
                start_time = time.time()
                self.run_command(f"{self.wgcf_path} generate")
                while time.time() - start_time < 6:
                    if os.path.isfile("wgcf-profile.conf"):
                        return
                    time.sleep(1)
                max_retry -= 1
